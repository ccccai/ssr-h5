<template>
  <div class="game-info-toast">
    ...
    <a-carousel v-if="list.length > 0" ref="myCarousel" effect="scrollx" :beforeChange="(from, to) => changeHandle(to)">
        ...
        <div class="game-info-content">
          ...
          <div class="top-panel">
            ...
            <div class="info">
              ...
              <!-- ***比赛中-未参赛, 已结束未报名 -->
              <div v-if="['2', '7'].includes(item.gameState)" class="middle-row">
                <div class="top-title">
                  比赛已结束
                </div>
                <div class="grade-tips">
            ...
            <template>
              <!-- ***比赛中-未参赛, 已结束未报名 -->
              <div v-if="['2', '7'].includes(item.gameState)" class="bottom-row">
                <div class="avatars">
                  <span>比赛奖金</span>
    ...
    <grade-toast
      ...
      :score="gradeScore"
      :total-user="gradeTotalUser"
      @close="() => showGradeToast=false"
    />
    <rank-toast
      :visible.sync="showRankToast"
      :rank="rankRank"
      :type="rankType"
      :change-rank="rankChange"
      :update-up="rankDate"
      @close="() => showRankToast=false"
    />
    <alert
...
export default {
  ...
  computed: {
    ...mapState({
      activeIndex: state => state.game.activeIndex
    }),
    gradeColor() {
      let color = '#fff'
  ...
  beforeRouteEnter(to, from, next) {
    // 表示从以下页面返回该页面，需要缓存之前的数据
    if (['Home'].includes(from.name)) {
      to.meta.showRank = true
    } else {
      to.meta.showRank = false
    }
    next()
  },
  created() {
  ...
  methods: {
    ...
    getGameInfoList() {
      ...
      this.$request.httpRequest({
        method: 'get',
        url: this.API.GetGameInfo,
        params: params,
        success: (data) => {
          // 获取下一期，还未开始的期数
          this.$store.dispatch('GetLastItem').then(last => {
            if (last) {
              this.list[0] = last
            }
            this.list.push(...data)
            this.changeHandle(this.activeIndex)
            this.$refs.myCarousel.goTo(this.activeIndex, true)
            this.showInfoToast = false
            if (this.$route.meta.showRank) {
              this.getRankingInfo()
            }
          }).catch(e => {
            this.infoType = 'error'
            this.infoText = '获取最新一期游戏数据出错'
          })
        },
        error: (e) => {
          this.infoType = 'error'
          this.infoText = e.hasOwnProperty('resultMessage') ? e.resultMessage : '获取赛况数据出错'
        }
      })
    },
    getRankingInfo() {
      ...
      this.$request.httpRequest({
        method: 'get',
        url: this.API.GetUserRankingInfo,
        params: params,
        success: (data) => {
          if (data.state === 0) {
            this.rankType = 3
          } else {
            if (data.changeRankind < 0) {
              this.rankType = 2
              this.rankChange = data.changeRankind
            } else if (data.changeRankind > 0) {
              this.rankType = 1
              this.rankChange = data.changeRankind
            } else {
              this.rankType = 4
            }
            this.rankRank = data.currentRanking
          }
          this.showRankToast = true
        },
        error: () => {}
      })
    },
    checkGrade(item) {
      ...
      this.gradeScore = item.score || 0
      this.gradeTotalUser = item.totalUser || 0
    ...
    toGameCondition(item) {
      if (['1', '2'].includes(item.gameState)) {
        this.alertType = 'error'
        this.alertText = '该赛事正在进行中，暂时不能查看赛况'
        this.showAlert = true
        return
      }
    ...
    changeHandle(val) {
      ...
      // 状态为比赛中和未开始的，需要进行倒计时
      if (!['1', '2', '-1'].includes(this.list[val].gameState)) {
        this.showInfoToast = false
        return
      }
      // 重新获取该期游戏详情，获取最新的倒计时时间
